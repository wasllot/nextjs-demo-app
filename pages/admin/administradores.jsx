...
export default function Administradores({ user }) {
	...
	const getAdmins = async () => {
		try {
			loaderDispatch({ type: "LOAD" });
			const { status, response } = await adminServices.getAdmins({ token: user.token });
			if (status === 200) {
	...
	const addCommerce = async ({ commerceName, ...data }) => {
		...
		try {
			setLoading(true);
			const { response, status } = await adminServices.addAdministrator({
				token: user.token,
				data: { ...data },
			});
			if (status === 200) {
		...
		>
			...
			>
				...
				>
					...
					<TextField
						...
						inputProps={{
							...register("userName", {
								required: "Ingrese el nombre de usuario admin",
							}),
						}}
						FormHelperTextProps={{ sx: { fontFamily: "Lato, sans-serif" } }}
			...
			<TableContainer component={Paper} sx={{ my: 4 }}>
				<Table>
					<TableHead>
						<TableRow>
							{/* <TableCell align="center">Terminal NFC</TableCell> */}
							<TableCell align="center">Usuario</TableCell>
							<TableCell align="center">Email</TableCell>
							{/* <TableCell align="center">Rol de usuario</TableCell> */}
					...
					<TableBody>
						{!loaderState.isLoading &&
							administrators.map((admin, index) => (
								<TableRow key={index}>
									{/* <TableCell align="center">{admin?.terminalNFC ?? 'N/A' }</TableCell> */}
									<TableCell align="center">{admin?.firstName} {admin?.lastName}</TableCell>
									<TableCell align="center">{admin?.email}</TableCell>
									{/* <TableCell align="center">{admin?.userRoll}</TableCell> */}
									<TableCell align="center">
										<Button
											...
											onClick={() => Router.push(`/admin/transacciones?userId=${admin._id}`)}
										>
											Ver transacciones
										</Button>
									</TableCell>
...
Administradores.getLayout = function getLayout(page) {
