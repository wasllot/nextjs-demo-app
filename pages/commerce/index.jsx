...
export default function LoginCommerce({}) {
	...
	const onSubmit = async ({ email, password }) => {
		try {
			...
			if (request.status === 200) {
				router.push(`/commerce/${response.commerce.name.toLowerCase()}`);
	...
	return (
		<Grid container sx={{ justifyContent: "center" }}>
			...
			<Grid
				...
				sx={{ padding: "35px 0", backgroundColor: "#0E1727", boxSizing: "content-box", zIndex: 0 }}
			>
				<Grid component={"img"} src="/logo-2.svg" alt="Cusica" width="80px" />
			</Grid>
			<Grid
				...
				container
				justifyContent="center"
				sx={{ zIndex: 0, margin: "30px", minHeight: "calc(100vh - 335px)" }}
				spacing={2}
				component={"form"}
			>
				<Grid item md={6} xs={12}>
					<Typography sx={{ fontWeight: "700", color: "#585858", textAlign: "center", fontSize: "1.5rem" }} component={"h1"} variant="h5">
						Inicia sesión
					</Typography>
				</Grid>
				...
				<Grid item md={4} sm={6} xs={8} container alignItems={"center"}>
					<Link
						...
						sx={{
							...
							justifyContent: { xs: "space-between", sm: "space-around", md: "space-around" },
						}}
					>
						<Fragment>
			...
			<Grid item md={12} xs={12} sx={{ position: "relative", height: 150 }}>
...
export const getServerSideProps = withIronSessionSsr(async function ({ req, res }) {
	...
	if (cookie) {
		...
		return {
			redirect: {
				destination: `/commerce/${name.toLowerCase()}`,
				permanent: false,
			},
		};
	}
